...
| 2026-05-23 | Claude Code | Full site audit via /run fetch-site — GSC data pulled via Windsor.ai MCP; found: /morocco-visa-from-india 404 (Google indexed, 11 impressions wasted), 34/37 country pages have 0 GSC impressions (not yet ranked), SOP page count discrepancy (SOP says 41, DESTS has 37), branded query 'evisas' at pos 5.84 not #1; saved full report to dashboard/result.json | Baseline audit to identify indexing gaps and broken pages |
| 2026-05-26 | Claude Code | GA4 full data pull via Chrome browser (officialevisas@gmail.com, property p499090813): 310 sessions/28d, 33 whatsapp_clicks (Key Events = 0 — not yet marked), 131 engaged sessions, 42.26% engagement rate; channels: Direct 60%, Organic 24.5%, Social 12.6%; 8 broken country pages found (france, indonesia, finland, nigeria, uganda, djibouti, tajikistan, moldova); encoding bug on 18 page views; saved to dashboard/ga4-report.json | Full intelligence pull combining site audit + GA4 + GSC |
| 2026-05-26 | Claude Code | Created 8 redirect pages for broken *-visa-from-india URLs: france→schengen, indonesia→bali, finland→schengen, moldova→schengen, nigeria/uganda/djibouti/tajikistan→homepage. All have meta-refresh + JS replace + canonical. | Fix 14 GA4 404 views + Google crawl errors on broken URLs |
| 2026-05-26 | Claude Code | Built dashboard analytics system: data-fetcher.js (GA4 + GSC + Facebook via google-auth-library + direct fetch), GET /analytics + GET /analytics/history + POST /analytics/refresh endpoints in server.js, full analytics panel in index.html with FB token expiry warning + Refresh Now button + week-over-week trends. Antigravity tweaks applied: lighter auth library, token expiry warning, history snapshots (analytics-history.json), manual refresh button. | Eliminate connector disconnects — data now cached locally every 6h, zero tokens |
| 2026-06-06 | Claude Code | CRITICAL SEO FIX: homepage loader was an early `return` that replaced the whole page, so the static HTML `<body>` contained ONLY the loader spinner — no h1, no nav, no country links. Confirmed via curl + SEObility ("Add H1", "few internal links / welcome page") + GSC (34/37 country pages 0 impressions, branded query at #5.8). Converted loader to a fixed-position overlay so full page is always in HTML. ALSO: country dest cards + footer dest links were `<div onClick>`/`href="#"` — converted to real `<a href="/{urlSlug}-visa-from-india">` (kept slide-in panel via preventDefault). ALSO: removed domain from homepage `<title>` → "Online Visa Services for 120+ Countries from India \| eVisas" (keyword-first). Build verified: h1 + 10 country links now in out/index.html. Zero visual change for users. | Homepage was SEO-invisible — Google saw only a spinner. This was the root cause of orphaned country pages and suppressed organic rankings. |
